...
export const mapKeys = <T, R>(
  ...
  Object.entries(obj).reduce(
    (acc, [key, value]) => ({
      ...acc,
      [key]: func(value, key as keyof T, obj),
    }),
    {} as Record<keyof T, R>,
  );
...
export const mapKeyNames = <T, R extends string>(
  ...
  Object.entries(obj).reduce(
    (acc, [key, value]) => ({
      ...acc,
      [func(key as keyof T, value, obj)]: value,
    }),
    {} as Record<R, T[keyof T]>,
  );

type Await<TPromise extends Promise<any>> =
  TPromise extends Promise<infer Value> ? Value : never;

export const resolveAllKeys = async <T extends Record<string, Promise<any>>>(
  obj: T,
) => {
...
export const objMerge = <T extends any[]>(objs: [...T]) =>
  objs.reduce((acc, obj) => ({ ...acc, ...obj }), {}) as UnionToIntersection<
    T[number]
  >;
...
export const arrayToObject = <V extends string, T extends Record<string, V>, R>(
  ...
  objs.reduce(
    (acc, obj) => ({ ...acc, [obj[keyedBy]]: mapFn(obj) }),
    {} as Record<V, R>,
  );
...
export const flatMap = <T, U>(arr: T[], fn: (a: T) => U[]) =>
  flatten(arr.map(fn));
